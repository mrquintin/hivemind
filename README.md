...
The repo pushes to **https://github.com/mrquintin/hivemind**. After accepting Cursor’s edits (“Keep all”):

- **From Cursor:** Run the task **“Sync to GitHub (after Keep all)”** (Command Palette → “Tasks: Run Task” → choose it), or bind a shortcut to that task in Keyboard Shortcuts.
- **From terminal:** `./scripts/sync-to-github.sh`

This commits all current changes with message “Sync: Cursor edits” and pushes to `origin` (main or your current branch). A post-commit hook also pushes after any manual `git commit`.

**If you use GitButler:** the sync script only works on normal branches. On `gitbutler/workspace`, run `but teardown` to switch to `main`, then use the script (or use GitButler’s own commit/push).
