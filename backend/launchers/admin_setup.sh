...
# =============================================================================
# Hivemind Admin Setup
# =============================================================================
# Installs prerequisites and runs the Admin app.
...
LOG_FILE="$LOG_DIR/admin_setup.log"
CONFIG_FILE="$HOME/.hivemind/admin_config"
...
clear_caches() {
  ...
  if [ -d "$ROOT_DIR/admin/src-tauri/target" ]; then
    log_info "Clearing Rust build cache..."
    rm -rf "$ROOT_DIR/admin/src-tauri/target"
  fi
  
  # Clear Vite cache
  rm -rf "$ROOT_DIR/admin/node_modules/.vite" 2>/dev/null || true
  
  # Clear npm cache for this project
  rm -rf "$ROOT_DIR/admin/node_modules/.cache" 2>/dev/null || true
  
  # Clear Tauri dev cache
  rm -rf "$ROOT_DIR/admin/src-tauri/target/debug/.fingerprint" 2>/dev/null || true
  ...
  rm -rf "$HOME/Library/WebKit/com.thenashlab.hivemind.admin" 2>/dev/null || true
  rm -rf "$HOME/Library/Caches/com.thenashlab.hivemind.admin" 2>/dev/null || true
...
test_server_connection() {
  ...
  if curl -s --connect-timeout 5 "$VITE_API_URL/health" >/dev/null 2>&1 || \
    ...
    echo ""
    echo "Make sure:"
    echo "  1. The cloud server is running (use Cloud Setup.app)"
...
launch_admin() {
  log_step "Launching Hivemind Admin..."
  cd "$ROOT_DIR/admin"

  echo ""
  log_info "Starting Admin app (connected to $VITE_API_URL)..."
  echo ""
...
main() {
  echo ""
  echo "=============================================="
  echo "       HIVEMIND ADMIN SETUP"
  echo "=============================================="
  echo ""
  echo "This will install all prerequisites and launch"
  echo "the Hivemind Admin application."
  echo ""
  ...
  echo ""
  echo "=============================================="
  echo "   SETUP COMPLETE - LAUNCHING ADMIN"
  echo "=============================================="
  echo ""
