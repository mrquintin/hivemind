...
MARKER="$ADMIN_DIR/.hivemind_installed"
TARGET_DIR="$ADMIN_DIR/src-tauri/target"

# Show native macOS notification instead of tkinter splash (avoids crash on machines without python-tk)
osascript -e 'display notification "Starting admin console..." with title "Hivemind Admin"' 2>/dev/null || true

echo ''
echo '╔══════════════════════════════════════════════════════════════════╗'
echo '║           HIVEMIND ADMIN APP LAUNCHER                           ║'
echo '╚══════════════════════════════════════════════════════════════════╝'
echo ''
...
# Run with retry on build failure (corrupted artifacts)
run_tauri() {
